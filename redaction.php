...
<div style="text-align:center;">
    <h2 style="margin-top: 20px;">Redaction d'un Article</h2><br />
        <p>Bienvenue dans la rédaction d'un article sur le site, vous avez obtenu les droits de redacteurs, et vous pouvez donc rediger un article. Mais il y a des regles à respecter :<br />- Pas d'insultes<br />- Pas de vulgarit&eacutes non-n&eacutecessaires<br />- Les &eacuteloges au administrateurs ou modérateurs ne sont pas interdites.<br />Le BBcode suivant est disponible : - [g] "texte" [/g] (mettre en <?php echo code('[g]gras[/g])') ?> - [i] "texte" [/i] (mettre en <?php echo code('[i]italique[/i])')?>- [s] "texte" [/s] (mettre en <?php echo code('[s]soulign&eacute[/s])')?>- [t] "texte" [/t] (mettre en <?php echo code('[t]barr&eacute[/t])')?>-[url] "lien" [/url] (Mettre un lien) -[img] "lien d'une image" [/img] (Mettre une image)</p>
    <div>
    </div>
    <form method="post" name="redaction">
        ...
        <textarea type="textarea" name="texte" value="<?php if(isset($redacContenu)){echo $redacContenu;} ?>" placeholder="Contenu de l'article" style="max-width:1000px;width:80%;max-height:500px;height:250px;margin-top:15px;"></textarea><br />
